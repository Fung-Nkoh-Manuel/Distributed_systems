...
class EnhancedNodeServer:
    ...
    def start(self, host='localhost', port=0):
        ...
        # Start node server
        actual_port = self.node.start(host, port)
    ...
    def _notify_network_file_created(self, file_id: str, file_name: str, file_size: int):
        ...
        try:
            ...
            if response.get('success'):
                print(f"✅ File {file_name} registered with network controller")
            else:
                print(f"⚠️  Warning: File {file_name} not registered with network: {response.get('error')}")
    ...
    def _notify_network_file_deleted(self, file_id: str, file_name: str):
        ...
        try:
            ...
            if response.get('success'):
                print(f"✅ File {file_name} unregistered from network controller")
            else:
                print(f"⚠️  Warning: File {file_name} not unregistered from network: {response.get('error')}")
